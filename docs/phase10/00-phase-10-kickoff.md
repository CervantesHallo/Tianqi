# Phase 10 Kickoff — 工程化与协作基础（启程战）

> **执行时间**：2026-05-02
> **类型**：Phase 10 启程战（不算 Step；K.4 重新命名落地）
> **本 Kickoff 性质**：Phase 启程级拆两阶段流程实战（PHASE_DESIGN v1 → v2 → APPROVE → PHASE_IMPLEMENT）；零业务代码 / 零测试增量；纯设计 + 文档落地
> **状态**：完成
...
| Phase 10 Step 数 | **7 Step + 1 Kickoff = 8 总数**（K.3 方案 B：不合并 Step 1/2）|
| ADR-0003 Status | **`In Progress (Phase 10 kickoff started 2026-05-02, ...)`** |
| 拆两阶段流程实战次数 | **第 3 次**（前两次：Phase 9 / Step 6 SagaOrchestrator + Step 14 CrossSagaCoordination；本次首次 Phase 启程级）|
| 业务代码增量 | **0**（克制原则——Phase 10 不构建业务代码）|
| 测试增量 | **0**（接近 0；最多允许 B 工程化测试）|
| 错误码增量 | **0**（惯例 K 严守）|
| 包数增量 | **0**（不引入新 package）|
| KI 状态变更 | KI-P8-001 修复责任 Phase: `Phase 10` → **`Phase 13+ TBD`**（K.1 修正补丁）|

**Phase 10 / Step 1 起步指令将由独立 Step 起草指令承接**（不在本 Kickoff 范围）。
...
| 规则 / 惯例 | 触发情况 |
|---|---|
| 元规则 B（接口签名冻结） | **严守** — 本 Kickoff 仅文档变更；不触碰任何 Phase 1-9 已锁定签名（git diff zero 跨 Phase 1-9） |
| 元规则 P（不主动引入第三方依赖）| **严守** — 0 新增第三方依赖（`pnpm-lock.yaml` 零变动）|
| 元规则 Q（强制开局动作）| **沿用 Phase 9 模式** — Phase 10 启程级首次实战；Step 1-7 各自重复实战（每 Step 4 项基础 + 工程化主题专属核查）|
| 惯例 K（错误码命名空间扩展）| **严守** — 0 新错误码 |
| 惯例 M（ADR 增量追写）| **沿用 Phase 9 模式** — ADR-0003 自此进入 Phase 10 内部 Step 增量追写阶段（Step 1-7 各自完成时追加） |
| §4.8 编译期硬约束 | **严守** — 本 Kickoff 不触碰 packages/domain；ESLint 规则零违规 |
| 拆两阶段流程 | **第 3 次实战 + 首次 Phase 启程级**（前两次：Phase 9 / Step 6 + Step 14；本次 Phase 启程级 vs 之前 Step 启程级）|
...
### F.4 Phase 9 closure 隐藏 typecheck 缺陷发现（诚实留痕）
...
**本 Kickoff 处置裁决**（基于 Tianqi 工程纪律"诚实评估"原则）：

1. **不在 Phase 10 Kickoff 修复**——修复属测试代码变更，违反 Phase 10 主题专属边界（工程化与协作基础，非测试代码维护）+ 元规则 B（涉及 Phase 9 / Step 16 测试文件）+ 克制原则（Kickoff 0 测试增量）
2. **登记为 Phase 10 / Step 3 前置依赖**——Step 3 主题"CI 强制门禁含 typecheck"必须前置修复此缺陷以让 CI 可启用；这在 Phase 10 工程化主题边界内合理；不创建独立 KI（避免与 Step 3 责任冗余）
3. **诚实留痕在 ADR-0003 + 本启程记录 + 00-phase1-mapping.md + KNOWN-ISSUES.md**（4 处明示）以让 Phase 10 / Step 3 起草指令承接时有清晰前置依赖说明
4. **Phase 9 closure 验证流程改进建议**：未来 Phase 收官 Step 验证流程必须显式含 `pnpm typecheck`（沿用 Phase 8 / Step 19 同类纪律但 Phase 9 / Step 19 漏掉的部分）；Phase 10 / Step 3 CI workflow 强制 typecheck 是结构性补救

**为什么不创建独立 KI（譬如 KI-P10-002）？**
- Step 3 主题已含修复责任（CI 强制门禁含 typecheck 必先修复缺陷才能启用）
- 创建独立 KI 与 Step 3 责任冗余违反"克制 > 堆砌"
- Phase 10 / Step 3 起草指令明示前置依赖即可（沿用 Phase 9 / Step 17 拒绝候选 6 不重复创建 KI 同精神）
...
- **Step 3 起草指令**：CI 强制门禁（GitHub Actions workflow + lint/typecheck/test/coverage + 84% 起步门槛）；性质工程化基础设施；**可能拆两阶段**（CI 配置冻结后约束 Step 4-7）
...
- **Step 7 起草指令**：Phase 10 CLOSED + CHANGELOG + CI 门槛升级 84% → 85%；性质收官；预期单阶段流程

**Step 1 起草由独立指令启动**（不在本 Kickoff 范围）。
...
**Phase 10 Kickoff 完成 — 2026-05-02 🚀**

拆两阶段流程第 3 次实战 + Phase 启程级首次实战；v1 → v2 修订证明 Phase 启程级用户审视的实证价值；Phase 10 / Step 1 由独立指令承接。
