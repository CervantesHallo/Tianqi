# Phase 11 / Step 0.5 v2 — PHASE_DESIGN 草案

> **状态**：第一阶段（PHASE_DESIGN）草案 **v2 修订完成**；等待用户 APPROVE。
> **拆两阶段流程第 8 次实战 / 元规则 Q v3 模板第 13 次实战**
> **v2 修订**：L.5 ci.yml image `apache/kafka:3.7.0` → `apache/kafka:3.7`（与 Step 0 `postgres:16-alpine` major.minor 约定一致）；L.1/L.2/L.3/L.4 直接采纳；ADR-0004 §D 边界澄清段由 AI 在 PHASE_IMPLEMENT 实地撰写。
...
**注意**：`apache/kafka:3.7` 镜像具体 KRaft 启动参数 + health-cmd 命令路径需要 PHASE_IMPLEMENT 实测验证（不同 Kafka 镜像版本路径可能略不同）。**v2 修订**：image 用 major.minor `3.7`（不固定 patch；与 Step 0 `postgres:16-alpine` 模式一致；自动 latest patch within 3.7.x）。
...
ci.yml test + coverage 双 job 加 `services.kafka`（与 services.postgres 共存；4 jobs 维持）。apache/kafka:3.7 KRaft 单 service 模式（不需 Zookeeper；与 Step 0 单 service 模式协调）。**镜像版本约定与 Step 0 一致**：major.minor `3.7`（不固定 patch；与 `postgres:16-alpine` 同精神；自动 latest patch within 3.7.x）。env var `TIANQI_TEST_KAFKA_BROKERS=localhost:9092`（裁决 K.2 沿用既有约定）。health-cmd `kafka-broker-api-versions.sh` + 显式 nc 端口探测双层防御（K.5）。
...
### ADR-0004 §D 测试 vs 业务代码 vs testkit 边界澄清（Step 0.5 实战延伸；PHASE_IMPLEMENT 实地撰写）

如 PHASE_IMPLEMENT 实测触发以下任一情况，追加本段（约 12-15 行）：

```markdown
## §D 测试 vs 业务代码 vs testkit 边界澄清（Step 0.5 实战延伸）

Step 0.5 PHASE_IMPLEMENT 实测可能发现 3 类边界情况：

1. **测试 vs adapter 默认语义不一致**：删除测试（不强加 adapter；如 L.1
   from_beginning / L.2 per_case_id）
2. **业务代码工程缺陷**：修复 + §D 沿用边界澄清（如 K.9 实测必要）
3. **testkit 设计遗漏**：补齐 + §D 边界澄清（如 persistent-notification-
   contract.ts 缺失）

3 类不在"修业务代码"禁令范围内，符合 Step 0 §D 边界（领域 + 应用 + 策略
层业务逻辑严守 vs adapter 实现 / testkit / 测试设计可改）。

[Step 0.5 实测触发的具体类型由 AI 在 PHASE_IMPLEMENT 实地填入]
```

由 AI 在 PHASE_IMPLEMENT 实地裁决是否追加（如三类全未触发→不追加）+ 实测填入具体触发实例。
...
| 风险 | Fallback |
|------|---------|
| apache/kafka:3.7 镜像启动参数与文档可能略不同 | PHASE_IMPLEMENT 实测验证 + 如有差异调整 yaml |
...
## L. 核心未决判断 — v2 处置（用户 v1 APPROVE + 1 项必做修订 + 4 项采纳）

### L.1 P1 from_beginning 测试 — ✅ PHASE_IMPLEMENT 实测决定

PHASE_IMPLEMENT 实测 adapter Consumer 默认行为；如 `fromBeginning: false` → 删除此测试（不强加 adapter 配置）+ ADR-0004 §D 边界澄清留痕（**"测试 vs adapter 默认语义不一致"分类**）。

### L.2 P3 per_case_id 测试 — ✅ PHASE_IMPLEMENT 实测决定

PHASE_IMPLEMENT 实测 adapter 默认 partitioner；如不保证顺序 → 删除或改为 `single_partition_preserves_order` + ADR-0004 §D 边界澄清留痕（**"测试 vs adapter 默认语义不一致"分类**）。

### L.3 persistent-notification-contract.ts 文件位置 — ✅ 锁定 `packages/adapters/adapter-testkit/src/`

与 Phase 8 既有 4 个 persistent contract（event-store / saga-state-store / dead-letter-store / config）同位置。补齐 Phase 8 设计疏漏 + ADR-0004 §D 边界澄清留痕（**"testkit 设计遗漏补齐"分类**）。

### L.4 业务代码修改 budget — ✅ 不预设（§B.1.E 防御应用边界严守）

PHASE_DESIGN 不预判；PHASE_IMPLEMENT 实测决定；如必须 → ADR-0004 §D 边界澄清沿用 Step 0 模式（**"业务代码工程缺陷"分类**）。**不预设 budget** 让"过度防御反伤工程纪律"原则严守（§B.1.E）。

### L.5 Kafka 镜像版本 — ✅ v2 必做修订：`apache/kafka:3.7`（major.minor）

**v1 草案 `apache/kafka:3.7.0` → v2 修订 `apache/kafka:3.7`**。理由：
- 与 Step 0 `postgres:16-alpine` major.minor 约定一致
- 自动 latest patch within 3.7.x
- 镜像版本约定的工程纪律统一

ADR-0004 Step 0.5 段 S0.5.1 已明示"镜像版本约定与 Step 0 一致"。
