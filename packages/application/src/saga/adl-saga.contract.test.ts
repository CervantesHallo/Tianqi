// Phase 9 / Step 11 — adl-saga 契约挂载测试。
//
// 一行挂载 Step 2 的 defineSagaContractTests，验证 ADLSaga 内部使用的
// SagaOrchestrator 配置（在 Sprint H 业务模块内组装）仍满足 Sprint F
// 17 个契约 it（§4 5 条接口纯度约束）。
//
// 这是 Phase 9 **第二次在业务 Saga 上挂载 Sprint F 契约**——证明业务
// 模板从 Step 10 LiquidationSaga 复制到 Step 11 ADLSaga 后契约仍守住。
// Sprint H 模板可复用性的关键证据：业务复杂度上升（多账户）但契约不破。
//
...
//
// 关键证据：本 driver 使用与 ADLSaga 内部组装编排器相同的
// createSagaOrchestrator 调用 + 相同的 in-memory 持久化基础设施。当
// ADLSaga 通过 SagaOrchestrator 调度业务 step（含多账户循环）时，编排
// 器仍满足 Sprint F 17 契约 it——多账户复杂度对契约透明。
// ============================================================

let driveCounter = 0;

const createADLSagaContractSubject = (): SagaContractSubject => {
  ...
  const drive = async (
    ...
    try {
      ...
      const invocation: SagaInvocation<unknown> = {
        sagaId: createSagaId(`saga-adl-contract-${stamp}`),
        traceId: createTraceId(`trace-adl-contract-${stamp}`),
        correlationId: createCorrelationId(`corr-adl-contract-${stamp}`),
      ...
      if (!result.ok) {
        throw new Error(
          `[adl-saga contract] runSaga unexpectedly returned err: ${result.error.message}`
        );
      }
...
// ============================================================
// 一行挂载 Step 2 的 17 契约 it（Phase 9 第二次在业务 Saga 上挂载；
// Step 11 是 Sprint H 模板可复用性的关键证据）
// ============================================================
