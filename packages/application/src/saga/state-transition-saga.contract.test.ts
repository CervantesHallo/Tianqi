// Phase 9 / Step 13 — state-transition-saga 契约挂载测试。
//
// 一行挂载 Step 2 的 defineSagaContractTests，验证 StateTransitionSaga
// 内部使用的 SagaOrchestrator 配置（在 Sprint H 业务模块内组装）仍满足
// Sprint F 17 个契约 it（§4 5 条接口纯度约束）。
//
// 这是 Phase 9 **第四次在业务 Saga 上挂载 Sprint F 契约**——证明业务
// 模板从 Step 10/11/12 复制到 Step 13 后（业务复杂度极低，4 step 紧凑
// 模式，0-N 个 Engine 动态消费）契约仍守住。**Sprint H 模板纪律极限考
// 验通过**：高复杂度（Step 11 ADL）+ 低复杂度（Step 12 InsuranceFund）
// + 极限低复杂度（Step 13 StateTransition）三个量级都守住，模板真正
// 可复用。
//
...
//
// 关键证据：本 driver 使用与 StateTransitionSaga 内部组装编排器相同的
// createSagaOrchestrator 调用 + 相同的 in-memory 持久化基础设施。当
// StateTransitionSaga 通过 SagaOrchestrator 调度业务 step（4 step 紧凑
// 模式，0-N 个 Engine 动态消费）时，编排器仍满足 Sprint F 17 契约 it
// ——业务复杂度低于 Step 10-12 同样契约不破，证明 Sprint H 模板纪律
// 极限考验通过。
// ============================================================

let driveCounter = 0;

const createStateTransitionSagaContractSubject = (): SagaContractSubject => {
  ...
  const drive = async (
    ...
    try {
      ...
      const invocation: SagaInvocation<unknown> = {
        sagaId: createSagaId(`saga-state-transition-contract-${stamp}`),
        traceId: createTraceId(`trace-state-transition-contract-${stamp}`),
        correlationId: createCorrelationId(`corr-state-transition-contract-${stamp}`),
      ...
      if (!result.ok) {
        throw new Error(
          `[state-transition-saga contract] runSaga unexpectedly returned err: ${result.error.message}`
        );
      }
...
// ============================================================
// 一行挂载 Step 2 的 17 契约 it（Phase 9 第四次在业务 Saga 上挂载；
// Step 13 是 Sprint H 模板纪律极限考验通过的关键证据）
// ============================================================
